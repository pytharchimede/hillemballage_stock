  ...
  function readCookieToken() {
    try {
      ...
      for (var i = 0; i < ca.length; i++) {
        ...
        const cards = data.map(function (c) {
          const photo = c.photo_path
            ? `<img src="${resolveImg(c.photo_path)}" alt="${escapeHtml(c.name || 'Client')}" class="avatar">`
          ...
          const balance = typeof c.balance !== 'undefined' ? parseInt(c.balance,10) || 0 : 0;
          const balFmt = new Intl.NumberFormat().format(Math.abs(balance)) + ' FCFA';
          const balLabel = balance === 0 ? '<span class="muted">Solde: 0 FCFA</span>' : (balance > 0 ? `<span style="color:#b50000;font-weight:700">Solde dû: ${balFmt}</span>` : `<span style="color:#1a7f37;font-weight:700">Avoir: ${balFmt}</span>`);
          return `
            <div class="card-client" data-id="${c.id}">
              <div class="cl-header">${photo}</div>
              <div class="cl-body">
                <div class="cl-name">${escapeHtml(c.name || 'Client')}</div>
                <div class="cl-phone">${tel ? `<i class='fa fa-phone'></i> ${escapeHtml(tel)}` : '<span class="muted">Téléphone non renseigné</span>'}</div>
                <div class="cl-balance">${balLabel}</div>
              </div>
              <div class="cl-actions">
                <a class="btn secondary" ${tel ? '' : 'disabled aria-disabled="true"'} href="${telHref}" title="Appeler"><i class="fa fa-phone"></i></a>
