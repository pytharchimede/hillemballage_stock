(function () {
  // Resolve route base robustly
  const form = document.getElementById("supplier-form");
  let routeBase = "";
  if (form && form.getAttribute("data-route-base")) {
    routeBase = form.getAttribute("data-route-base");
  } else {
    // Fallback: infer from current location (assumes /hill_new/public prefix)
    try {
      const loc = window.location.pathname; // e.g. /hill_new/public/suppliers/new
      const idx = loc.indexOf("/suppliers/");
      routeBase = idx > 0 ? loc.substring(0, idx) : loc.replace(/\/[^/]*$/, "");
    } catch {}
  }
  ...
  // Leaflet map + marker (optional: only if a container exists)
  let map = null;
  let marker = null;
  const supplierMapEl = document.getElementById("supplier-map");
  const mapEl = document.getElementById("map");
  const mapContainerId = supplierMapEl ? "supplier-map" : mapEl ? "map" : null;
  if (typeof L !== "undefined" && mapContainerId) {
    map = L.map(mapContainerId);
    marker = L.marker([0, 0], { draggable: true }).addEventListener
      ? L.marker([0, 0], { draggable: true }).addTo(map)
      : L.marker([0, 0], { draggable: true }).addTo(map);
    ...
    geoSearch.addEventListener("input", () => {
      ...
      geoTimer = setTimeout(() => {
        fetch(
          ...
          .catch(() => {
            if (geoSuggest) geoSuggest.style.display = "none";
          });
      }, 300);
    });
  geoSuggest &&
